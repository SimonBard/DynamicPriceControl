...
class kostal_modbusquery:
    def __init__(self):
        ...
        self.Adr[1042] = [1042,"Minimum SOC", "Float"]

        #self.Adr[1042] = [1042,"Minimum SOC", "WriteR32"]

        self.Adr[1044] = [1044,"Maximum SOC", "Float"]
    ...
    def WriteR32(self,myadr_dec,value):
        ...
        print ("From  subroutine - In theory .... - I should get the value back that we pushed ", value ,"and got back", myreadregister)
        print("Register I wrote",mywriteregister)
        print ("if values differ, we need to set the battery management to external")
    ...
    def ReadR32(self, myadr_dec):
        ...
        myreadregister = BinaryPayloadDecoder.fromRegisters(myreadregister.registers, byteorder=Endian.Big, wordorder=Endian.Little)
        myreadregister =round(myreadregister.decode_32bit_float(),2)
        #print ("Value was ", myreadregister)
        return myreadregister
    ...
    def load_battery(self, ladeleistung):
        print('Battery should load now for 1 hour with the given value')
        # It is probably better to put the time part (1 hour) to the hourly file
        delay = 60*60 # For 60 minutes delay 
        close_time = time.time()+delay
        inst = lib.mymqtt.Mqtt()
        while True:
            if self.getGridPower()<50:
            # battery value is only written if no power is sent to grid. Means battery to grid not possible.
            # probably better to handle this part outside of the function    
                self.WriteR32(1034, ladeleistung)
                inst.mqtt_publish_battery(ladeleistung)
                time.sleep(30) 
                if time.time() > close_time:
                    break
            else: 
                print('energy is sent to grid, therefore battery should take over control again')

    def load_battery_1min(self, chargingPower):
        print('Battery should load now for 1 minute with the given value')
        self.WriteR32(1034, chargingPower)
        

    def set_minsoc(self, minsoc):
        self.WriteR32(1042, minsoc)
        # probably ts not R32, but ?. But the number is read correctly

    def get_minsoc(self):
        #self.ReadR32(1042)
        #self.run()
        return self.ReadR32(1042)
    
    def get_currentsoc(self):
        #self.ReadR32(1042)
        #self.run()
        return self.Adr[514][3]
    ...
    def getTotalYield(self):
        return (self.Adr[320][3]/1000)

    def getTotalHomeConsumption(self):
        return (self.Adr[118][3]/1000)

    def getGridPower(self):
        # positive value mean power to grid, negative values mean power from grid to home
        #self.run()
        #print('Grid Power is: ' , self.Adr[252][3])
        return (self.Adr[252][3])
...
def main():
    try:
        ...
        mybattery = kostal_modbusquery()
        mybattery.run()
        print("Gesamtertrag: ", mybattery.Adr[320][3])
        print("Gesamtverbrauch: ", mybattery.Adr[118][3])
        print("Current SOC: ", mybattery.Adr[514][3])
        print('Grid Power is: ' , mybattery.getGridPower())
        today = datetime.now().strftime('%Y-%m-%d')
        #print(today)
        SManager = lib.manager.Manager()
        SManager.write_totalyield(today, mybattery.Adr[320][3]/1000)
        SManager.write_total_home_consumption(today, mybattery.Adr[118][3]/1000)
        print('TotalAC2Grid is: ', mybattery.getTotalAC2Grid())
        #mybattery.load_battery_1min(-5000.0)
        # ts= time.time()
        # Kostalquery.run()
        # te = time.time()
        ...
        #print("minSOC right now is: ", mybattery.get_minsoc())

        #minSOC = 35.0
        #mybattery.set_minsoc(minSOC)
        # inst = mymqtt.Mqtt()
        # message = 'Battery should load now to ' + str(minSOC)
        # inst.mqtt_publish_battery(message)
